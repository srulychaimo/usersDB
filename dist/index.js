    ...
    try {
        ...
        for (const query of questions) {
            let answer = await rl
                ...
                .then((data) => validateType(query.type, data, query.length, query.keys ? query.keys : null));
            while (!answer) {
                answer = await rl
...
const searchUserById = async () => {
    try {
        ...
        const rl = readline.createInterface({ input, output, terminal: false });
        const searchId = await rl.question("Whats the user id of the user your looking for ? ".yellow);
        let index = [];
        ...
        let action = await rl.question("What do you want to get ('all' | 'filed') ? ".yellow);
        while (!action || (action !== "all" && action !== "filed")) {
            action = await rl.question("What do you want to get ('all' | 'filed') ? ".yellow);
        }
        await handleAction(action, index);
        if (!index.length) {
            console.error(`cannot find a user with the id: ${searchId}, please try again!`.red);
        }
        runProgram();
        rl.close();
    }
...
const runProgram = async () => {
    const rl = readline.createInterface({ input, output, terminal: false });
    const action = await rl.question("Hello and welcome, what would you like to do (add || search)? ".blue);
    if (action === "add") {
